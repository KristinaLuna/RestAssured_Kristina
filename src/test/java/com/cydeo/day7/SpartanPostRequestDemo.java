 ...
 */
    ...
    public void test3() {
        ...
        Response response = given().accept(ContentType.JSON).log().all() // what we are asking from api which is JSON response
                .and()
                .contentType(ContentType.JSON).log().all() //what we are sending to api, which is JSON request body
                .body(spartan)
                .when()
                .post("/api/spartans");
    ...
    public void test4() {
      ...
      int idFromPost =  given()
        ...
        Spartan spartanGet = given()
                .accept(ContentType.JSON).log().all()
                .and().pathParam("id", idFromPost)
                .when().get("/api/spartans/{id}")
                .then().extract().response().as(Spartan.class);

        assertThat(spartanGet.getName(), is(spartanPost.getName()));
        assertThat(spartanGet.getGender(),is(spartanPost.getGender()));
        assertThat(spartanGet.getPhone(),is(spartanPost.getPhone()));
